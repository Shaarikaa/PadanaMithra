...
import { GraduationCap, Mail, Lock, ArrowLeft, Eye, EyeOff, CircleAlert as AlertCircle, Users } from 'lucide-react';
import { useApp } from '@/lib/AppContext';
import { Button } from '@/components/ui/button';
import { Input } from '@/components/ui/input';
import { Label } from '@/components/ui/label';
import { parentLogin } from '@/lib/parentService';
...
export function ParentLoginPage() {
  ...
  const handleSubmit = async (e: FormEvent) => {
    ...
    setLoading(true);
    const result = await parentLogin(email.trim(), password);
    setLoading(false);
    if (!result.success) {
      setError(result.error ?? 'Login failed.');
      return;
    }
    navigate({ name: 'parent-dashboard' });
  };
    ...
    <div className="flex min-h-screen flex-col bg-white lg:flex-row">
      <div className="relative hidden flex-1 overflow-hidden bg-gradient-to-br from-indigo-600 via-indigo-600 to-violet-600 lg:flex lg:flex-col lg:justify-between lg:p-12">
        ...
        <div className="relative text-white">
          <h2 className="text-3xl font-bold leading-tight">Parent Dashboard</h2>
          <p className="mt-3 max-w-md text-indigo-100">
            Track your child's learning progress with monthly reports and real-time activity insights.
          </p>
        </div>
        <p className="relative text-sm text-indigo-200">Supporting your child's learning journey.</p>
      </div>
      ...
      <div className="flex flex-1 items-center justify-center px-4 py-10 sm:px-6">
        <div className="w-full max-w-sm">
          ...
          <div className="mb-6 flex items-center gap-2">
            ...
            <h1 className="text-2xl font-bold tracking-tight text-slate-900">Parent Login</h1>
          </div>
          <p className="mt-1.5 text-sm text-slate-500">Enter your credentials to view your child's progress.</p>
          ...
          <form onSubmit={handleSubmit} className="mt-8 space-y-5">
            ...
            <Button type="submit" disabled={loading} className="h-11 w-full bg-indigo-600 text-base hover:bg-indigo-700">
              {loading ? 'Logging in...' : 'Log in to Parent Dashboard'}
            </Button>
          </form>

          <div className="mt-6 rounded-xl border border-slate-100 bg-slate-50/50 p-4">
            <p className="text-xs text-slate-500">
              <strong className="text-slate-700">First time logging in?</strong> Use any password on your first login — it will be set as your password going forward.
            </p>
          </div>
        </div>
      </div>
